...
(function () {
  ...
  document.addEventListener("pw_cart_updated", (e) => {
    ...
    document
      .querySelectorAll(".cart-count, .cart-contents-count, [data-pw-cart-count]")
      .forEach((el) => {
  ...
  class ProductWidget {
    ...
    _render() {
      ...
      this.el.innerHTML = `
        ${s.filters || s.search ? `<div class="pw-toolbar">
    ...
    _renderProducts(products) {
      ...
      grid.innerHTML = products
        .map((p) => {
          ...
          if (isGrid) {
            return this._gridCard(p, isSimple, wishlisted);
          } else {
            return this._listCard(p, isSimple, wishlisted);
          }
        })
        .join("");
        ...
        <div class="pw-product-card pw-product-card--grid" data-id="${p.id}">
          <div class="pw-product-thumb">
            ...
            <button class="pw-wishlist-btn ${wishlisted ? "pw-wishlist-btn--active" : ""}"
              ...
              <svg viewBox="0 0 24 24" fill="${wishlisted ? "currentColor" : "none"}" stroke="currentColor" stroke-width="2">
    ...
    _listCard(p, isSimple, wishlisted) {
      const showWishlist = this.settings.wishlist;
    ...
    _renderPagination(pagination) {
      ...
      for (let i = 1; i <= total; i++) {
        if (total > 7 && i > 2 && i < total - 1 && Math.abs(i - current) > 1) {
          if (i === 3 || i === total - 2) html += `<span class="pw-page-ellipsis">…</span>`;
          continue;
        }
      ...
      container.querySelectorAll(".pw-page-btn:not([disabled])").forEach((btn) => {
        btn.addEventListener("click", (e) => {
